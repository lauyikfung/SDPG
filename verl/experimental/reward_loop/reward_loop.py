...
def migrate_legacy_reward_impl(config):
    ...
    # 5. answer_format migration
    # config.reward_model.answer_format -> config.reward.answer_format
    answer_format = config.reward_model.get("answer_format")
    if answer_format is not None:
        with open_dict(config.reward):
            config.reward["answer_format"] = answer_format

    # 6. sandbox_fusion migration
