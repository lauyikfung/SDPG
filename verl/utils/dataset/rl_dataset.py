...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.answer_format = config.get("answer_format", "answer_colon")
    ...
    TEACHER_CONTEXT_TOKEN = "[TEACHER_CONTEXT_TOKEN]"

    # Original instruction embedded in math-dapo prompts
    _ANSWER_COLON_INSTRUCTION = (
    ...
    def _apply_answer_format(self, messages: list) -> list:
        """Replace the answer-format instruction in user messages when answer_format='boxed'."""
        if self.answer_format == "answer_colon":
            return messages
        import copy as _copy
        new_messages = _copy.deepcopy(messages)
        for msg in new_messages:
            content = msg.get("content", "")
            if isinstance(content, str) and self._ANSWER_COLON_INSTRUCTION in content:
                msg["content"] = content.replace(
                    self._ANSWER_COLON_INSTRUCTION, self._BOXED_INSTRUCTION, 1
                )
        return new_messages

    def _split_teacher_context(self, messages: list) -> tuple[list, list | None]:
        """Split messages at [TEACHER_CONTEXT_TOKEN] into actor prompt and teacher prompt.

        The first user message content may contain '[TEACHER_CONTEXT_TOKEN]' followed by
        teacher context. The actor sees only the content before the token; the teacher sees
        the full content (including the context after the token).

        Args:
            messages: List of message dicts (role/content pairs).

        Returns:
            (actor_messages, teacher_messages): teacher_messages is None if no token found.
        """
        import copy as _copy

        token = self.TEACHER_CONTEXT_TOKEN
        for i, msg in enumerate(messages):
            content = msg.get("content", "")
            if isinstance(content, str) and token in content:
                actor_content, teacher_context = content.split(token, 1)
                # Actor sees only the part before the token
                actor_messages = _copy.deepcopy(messages)
                actor_messages[i] = {**msg, "content": actor_content}
                # Teacher sees actor prompt + teacher context (delimiter stripped)
                teacher_messages = _copy.deepcopy(messages)
                teacher_messages[i] = {**msg, "content": actor_content + teacher_context}
                return actor_messages, teacher_messages
    ...
    def __getitem__(self, item):
        ...
        raw_messages = self._apply_answer_format(raw_messages)
        actor_messages, teacher_messages = self._split_teacher_context(raw_messages)
        row_dict["raw_prompt"] = actor_messages

        if teacher_messages is not None:
            # Pre-tokenize teacher prompt (full prompt with context) and store as tensors.
            # The teacher prompt is left-padded to max_prompt_length to match actor prompt layout.
            ...
            # padding_side for prompts is left (consistent with AgentLoop)
            # transformers pads on the right by default; we need to left-pad manually.
            teacher_ids = teacher_enc["input_ids"][0]  # (max_prompt_length,)
            teacher_mask = teacher_enc["attention_mask"][0]  # (max_prompt_length,)
            # Convert right-padded to left-padded
            n_real = teacher_mask.sum().item()
            teacher_ids_left = torch.zeros_like(teacher_ids)
            teacher_mask_left = torch.zeros_like(teacher_mask)
            teacher_ids_left[-n_real:] = teacher_ids[:n_real]
            teacher_mask_left[-n_real:] = 1
            teacher_ids_left[:-n_real] = self.tokenizer.pad_token_id if self.tokenizer.pad_token_id is not None else 0
            row_dict["teacher_input_ids"] = teacher_ids_left
            row_dict["teacher_attention_mask"] = teacher_mask_left
