...
def rearrange_micro_batches(
    ...
    if dist.is_initialized() and same_micro_num_in_dp:
        # Synchronise the micro-batch count across all ranks in the data-parallel group
        # (or the default WORLD group when dp_group=None).  Without this, ranks with
        # different total sequence lengths produce different micro-batch counts; the ranks
        # that finish early exit the FSDP AllGather loop while other ranks are still inside
        # it, causing an NCCL collective mismatch → 600-second NCCL watchdog timeout.
