...
def _compute_effective_beta(beta: float, current_step: int, total_steps: int,
                            warmup_steps: int, decay_steps: int) -> float:
    """Compute the effective beta value applying optional warmup and decay schedules.

    Warmup: linearly increases beta from 0 → beta over the first ``warmup_steps`` steps.
    Decay:  linearly decreases beta → 0 over the last ``decay_steps`` steps of training.
    Both schedules are independent and can be used together; the decay multiplier is
    applied on top of the warmed-up value.  If ``total_steps == 0`` the decay is skipped.
    """
    effective = beta
    # Warmup: 0 → beta over first warmup_steps steps
    if warmup_steps > 0 and current_step < warmup_steps:
        effective = beta * (current_step / warmup_steps)
    # Decay: beta → 0 over last decay_steps steps
    if decay_steps > 0 and total_steps > 0:
        decay_start = total_steps - decay_steps
        if current_step >= decay_start:
            remaining = max(total_steps - current_step, 0)
            effective = effective * (remaining / decay_steps)
    return effective
...
def compute_sdpg_loss(
    ...
    log_prob: torch.Tensor,                # current actor log π_θ(y_t | x, y_{<t})
    kl_distill_per_token: torch.Tensor,    # full-vocab KL(π_θ(·|x) ‖ π_teacher(·|c,x)) per token
    ...
    Extends GRPO/DAPO with a full-vocabulary KL distillation term computed
    against the current actor using privileged context c:

        D_KL^(t) = Σ_{y'∈V} π_θ(y'|x,y_{<t}) · (log π_θ(y'|x,y_{<t}) - log π_teacher(y'|c,x,y_{<t}))

    This is precomputed in dp_actor.update_policy via a dedicated no-grad teacher
    forward pass and passed in as `kl_distill_per_token` of shape (B, T).

    The reward term uses DAPO dual-clip PPO. An optional alpha regularization
    term anchors the policy to the frozen reference model (π_old).
    """
    assert config is not None
    assert isinstance(config, ActorConfig)

    # --- DAPO-style dual-clip reward term ---
    clip_base = rollout_log_prob if rollout_log_prob is not None else old_log_prob
    negative_approx_kl = torch.clamp(log_prob - clip_base, min=-20.0, max=20.0)
    ...
    pg_clipfrac_lower = verl_F.masked_mean(
        torch.gt(clip_pg_losses1, pg_losses3).float() * (advantages < 0).float(), response_mask
    )
    pg_losses = torch.where(advantages < 0, torch.min(pg_losses3, clip_pg_losses1), clip_pg_losses1)

    # --- Alpha term: KL regularization against frozen reference model ---
    logw = log_prob - old_log_prob   # log(π_θ / π_old)
    inverse_w = torch.exp(-logw)     # π_old / π_θ

    alpha = config.policy_loss.get("alpha", 1e-4)
    kl_mode = config.policy_loss.get("kl_mode", "urkl")

    # --- Beta term: full-vocab KL distillation ---
    beta_base = config.policy_loss.get("beta", 1e-4)
    ...
    if config.policy_loss.get("beta_distill_positive_advantage_only", False):
        beta_term = beta_term * (advantages > 0).float()

    if kl_mode == "fkl":
        pg_losses = pg_losses + beta_term + alpha * inverse_w
    elif kl_mode == "rkl":
        pg_losses = pg_losses + beta_term + (alpha / 2) * (logw + 1) ** 2
    elif kl_mode == "ufkl":
        pg_losses = pg_losses + beta_term + alpha * (inverse_w + logw)
    elif kl_mode == "urkl":
        pg_losses = pg_losses + beta_term + (alpha / 2) * logw ** 2
    else:
        raise ValueError(f"Unknown kl_mode: {kl_mode}. Choose from fkl, rkl, ufkl, urkl")
    ...
    pg_metrics = {
        ...
        "actor/beta_effective": beta,
        "actor/kl_distill": verl_F.masked_mean(kl_distill_per_token.detach(), response_mask).item(),
    }
...
def compute_opsd_loss(
    log_prob: torch.Tensor,          # (B, T): log π_θ(y_t | x), current, WITH grad
    teacher_log_prob: torch.Tensor,  # (B, T): log π_ref(y_t | c, x), frozen ref model
    old_log_prob: torch.Tensor,      # (B, T): log π_rollout(y_t | x), step-start rollout
    response_mask: torch.Tensor,
    advantages: torch.Tensor,        # (B, T): GRPO advantage, for reward gate
    loss_agg_mode: str = "token-mean",
    config: Optional[ActorConfig] = None,
    **kwargs,
) -> tuple:
    """
    OPSD: On-Policy Self-Distillation (arxiv 2601.18734v3) + three stability fixes.

    w_t = clamp(log π_ref(y_t|c,x) - log π_θ_k(y_t|x), 0, 20)   [per-token weight]
    r_t = π_θ(y_t|x) / π_θ_k(y_t|x)
    L   = -w_t · min(r_t, clip(r_t, 1-ε_l, 1+ε_h))   on correct seqs only
    A_n = mean_t(w_t)   [sequence mean, logged only]

    Fix 1 — reward gate (GRPO adv > 0):
        Distillation signal alone is too noisy at low accuracy; gate ensures only
        correct trajectories are distilled.

    Fix 2 — clamp(0, 20) on advantage:
        Prevents A_n < 0 when student surpasses frozen teacher, which would push
        correct tokens down and cause reward collapse.

    Fix 3 — PPO ratio clip:
        Pure REINFORCE (loss = -A_n * log_prob) uses log_prob ≈ -10 at init
        (large magnitude), causing gradient scale mismatch that crashes training.
        PPO ratio form (loss = -A_n * ratio, ratio ≈ 1 at step start) keeps the
        loss near -A_n ≈ -0.003, a well-conditioned scale. The clip also bounds
        per-step policy movement across 64 mini-batches.

    Fix 4 — entropy_coeff=0.01 (in script):
        OPSD has only positive gradients. Without entropy regularization, entropy
        collapses monotonically → rollout groups become homogeneous → gate_ratio
        crashes to 0. entropy_coeff maintains rollout diversity.

    Note: PPO clip causes A_n to stay at ~0.003 (vs ~0.05 without clip) because
    it limits per-step policy movement, preventing the composition effect that
    evolves A_n upward as harder problems get solved. This is an acceptable
    trade-off given that removing the clip crashes training immediately.
    """
    assert config is not None
    assert isinstance(config, ActorConfig)

    # Reward gate: only distill on sequences where GRPO advantage > 0 (correct answers).
    # advantages shape: (B, T) — all tokens in a sequence share the same scalar.
    seq_adv = advantages[:, 0]                               # (B,)
    reward_gate = (seq_adv > 0).float().unsqueeze(-1)        # (B, 1)

    # Per-token distillation advantage: clamp(0, 20).
    # Using per-token weights (not sequence-mean) so gradient concentrates on answer tokens
    # where teacher has actual advantage, rather than being diluted by ~2000 context tokens.
    # nan_to_num: (-inf) - (-inf) = NaN for tokens where both models assign near-zero prob.
    # NaN * 0 (masked-out) is still NaN in IEEE 754 and propagates through masked_sum → pg_loss NaN.
    per_token_adv = (teacher_log_prob - old_log_prob).detach().nan_to_num(0.0).clamp(0.0, 20.0)  # (B, T)
    seq_len = response_mask.sum(dim=-1, keepdim=True).clamp(min=1).float()
    A_n = (per_token_adv * response_mask).sum(dim=-1, keepdim=True) / seq_len    # (B, 1) for logging only

    effective_mask = response_mask * reward_gate                                  # (B, T)

    # PPO ratio clip: stable loss scale.
    ratio = torch.exp(torch.clamp(log_prob - old_log_prob, min=-20.0, max=20.0)).nan_to_num(1.0)  # (B, T)
    ratio_clipped = ratio.clamp(1.0 - config.clip_ratio_low, 1.0 + config.clip_ratio_high)
    # Per-token weighting: gradient concentrated on tokens where teacher is more confident.
    loss_mat = -per_token_adv * torch.min(ratio, ratio_clipped)
    ...
    reward_gate_ratio = reward_gate[:, 0].mean().item()
    pg_metrics = {
        "actor/opsd_advantage":         verl_F.masked_mean(A_n.expand_as(response_mask), effective_mask).item(),
        "actor/opsd_reward_gate_ratio": reward_gate_ratio,
        "actor/opsd_ratio":             verl_F.masked_mean(ratio, effective_mask).item(),
    }
    return pg_loss, pg_metrics
...
def compute_rlsd_loss(
    old_log_prob: torch.Tensor,      # (B, T): log pi_old(y_t | x), rollout policy
    log_prob: torch.Tensor,          # (B, T): log pi_theta(y_t | x), current policy with grad
    teacher_log_prob: torch.Tensor,  # (B, T): log pi_teacher(y_t | c, x), stop-grad privileged context
    advantages: torch.Tensor,        # (B, T): GRPO sequence advantage broadcast to tokens
    response_mask: torch.Tensor,
    loss_agg_mode: str = "token-mean",
    config: Optional[ActorConfig] = None,
    rollout_is_weights: torch.Tensor | None = None,
    ...
    **kwargs,
    ...
    """
    RLSD: RLVR with Self-Distillation (arxiv 2604.03128v2).

    RLSD keeps the policy-gradient direction anchored to GRPO advantages and uses
    privileged self-distillation only as a stop-gradient magnitude reweighting:

        delta_t = sg(log pi_T(y_t | c, x) - log pi_old(y_t | x))
        w_t     = exp(sign(A) * delta_t)
        Ahat_t  = A * clip(w_t, 1 - eps_w, 1 + eps_w)

    A mixing coefficient lambda interpolates between standard GRPO and RLSD:

        A_mix_t = (1 - lambda) * A + lambda * Ahat_t

    The mixed token-level advantage is then used in the standard PPO/GRPO clipped
    surrogate. There is no auxiliary KL/distillation loss, so privileged context
    never enters the gradient direction.
    """
    assert config is not None
    assert isinstance(config, ActorConfig)
    ...
    delta = (teacher_log_prob - old_log_prob).detach().nan_to_num(0.0).clamp(min=-20.0, max=20.0)
    adv_sign = torch.sign(advantages.detach())
    evidence_weight = torch.exp((adv_sign * delta).clamp(min=-20.0, max=20.0))

    eps_w = config.policy_loss.get("rlsd_epsilon_w", 0.2)
    clipped_weight = evidence_weight.clamp(1.0 - eps_w, 1.0 + eps_w)

    lambda_base = config.policy_loss.get("rlsd_lambda", 0.5)
    lambda_decay_steps = config.policy_loss.get("rlsd_lambda_decay_steps", 50)
    if lambda_decay_steps > 0:
        lambda_scale = max(0.0, 1.0 - (current_step / lambda_decay_steps))
    else:
        lambda_scale = 1.0
    lambda_effective = lambda_base * lambda_scale

    rlsd_advantages = advantages * clipped_weight
    mixed_advantages = (1.0 - lambda_effective) * advantages + lambda_effective * rlsd_advantages

    negative_approx_kl = torch.clamp(log_prob - old_log_prob, min=-20.0, max=20.0)
    ratio = torch.exp(negative_approx_kl)
    ppo_kl = verl_F.masked_mean(-negative_approx_kl, response_mask)

    pg_losses1 = -mixed_advantages * ratio
    pg_losses2 = -mixed_advantages * torch.clamp(ratio, 1 - clip_ratio_low, 1 + clip_ratio_high)
    clip_pg_losses1 = torch.maximum(pg_losses1, pg_losses2)
    pg_clipfrac = verl_F.masked_mean(torch.gt(pg_losses2, pg_losses1).float(), response_mask)

    pg_losses3 = -mixed_advantages * clip_ratio_c
    clip_pg_losses2 = torch.min(pg_losses3, clip_pg_losses1)
    pg_clipfrac_lower = verl_F.masked_mean(
        torch.gt(clip_pg_losses1, pg_losses3).float() * (mixed_advantages < 0).float(), response_mask
    )
    pg_losses = torch.where(mixed_advantages < 0, clip_pg_losses2, clip_pg_losses1)
    ...
    weight_clipfrac = verl_F.masked_mean(
        ((evidence_weight < 1.0 - eps_w) | (evidence_weight > 1.0 + eps_w)).float(), response_mask
    )
    pg_metrics = {
        "actor/pg_clipfrac": pg_clipfrac.detach().item(),
        "actor/ppo_kl": ppo_kl.detach().item(),
        "actor/pg_clipfrac_lower": pg_clipfrac_lower.detach().item(),
        "actor/rlsd_delta": verl_F.masked_mean(delta, response_mask).item(),
        "actor/rlsd_weight": verl_F.masked_mean(evidence_weight, response_mask).item(),
        "actor/rlsd_weight_clipped": verl_F.masked_mean(clipped_weight, response_mask).item(),
        "actor/rlsd_weight_clipfrac": weight_clipfrac.detach().item(),
        "actor/rlsd_lambda": lambda_effective,
    }
    return pg_loss, pg_metrics
