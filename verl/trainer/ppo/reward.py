...
def load_reward_manager(config: DictConfig, tokenizer: Any, **reward_kwargs: Any) -> RewardManagerBase:
    ...
    if compute_score is None:
        ...
        answer_format = (
            OmegaConf.select(config, "reward.answer_format")
            or OmegaConf.select(config, "reward_model.answer_format")
            or OmegaConf.select(config, "reward.reward_model.answer_format")
            or "answer_colon"
        )
        if sandbox_url:
            ...
            final_compute_score = partial(default_compute_score, answer_format=answer_format)
