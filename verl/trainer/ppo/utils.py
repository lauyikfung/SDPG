...
def need_reference_policy(
    ...
    _loss_mode = config.actor_rollout_ref.actor.policy_loss.get("loss_mode", "vanilla")
    is_sdpg = _loss_mode == "sdpg"  # sdpg needs frozen ref for alpha term
    is_opsd = _loss_mode == "opsd"  # paper: teacher = frozen πref (initial weights)
    return config.algorithm.use_kl_in_reward or config.actor_rollout_ref.actor.use_kl_loss or is_sdpg or is_opsd
