...
class PolicyLossConfig(BaseConfig):
    ...
    # SDPG hyperparameters
    beta: float = 1e-4      # KL distillation coefficient (teacher)
    alpha: float = 1e-4     # KL regularization coefficient (old model)
    kl_mode: str = "fkl"    # one of: fkl, rkl, ufkl, urkl
    # SDPG beta schedule: linear warmup from 0 → beta over the first beta_warmup_steps steps.
    # 0 = disabled (beta is constant from step 0).
    beta_warmup_steps: int = 0
    # SDPG beta schedule: linear decay from beta → 0 over the last beta_decay_steps steps.
    # 0 = disabled (no decay at end of training).
    ...
    opsd_teacher_temperature: float = 1.0
    # RLSD-specific hyperparameters (arxiv 2604.03128v2)
