...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        # skip_entropy=True: caller requests no entropy computation (e.g. SDPG/RLSD teacher log prob
        # only needs log probs, not entropy).  Skipping avoids holding the full [tokens, vocab_size]
        # logits tensor alive during softmax-entropy, which is the primary source of peak-memory
        # fragmentation that causes FSDP AllGather OOM on one rank (typically NCCL timeout).
        skip_entropy = data.meta_info.pop("skip_entropy", False)
        ...
        calculate_entropy = not is_lora and not skip_entropy
        with self.ulysses_sharding_manager:
            with adapter_ctx:
        ...
        # Free cached GPU memory to reduce fragmentation between computation phases
        # (old_log_prob → ref_log_prob → teacher_log_prob → actor_update).
        get_torch_device().empty_cache()
