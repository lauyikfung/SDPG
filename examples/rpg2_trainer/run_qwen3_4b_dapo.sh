#!/bin/bash
# DAPO training script for Qwen3-4B on math reasoning tasks.
# Implements DAPO (Song et al., 2025, arxiv 2503.14476):
#   - Clip-Higher: asymmetric clip ratios (clip_ratio_low=0.2, clip_ratio_high=0.28)
#   - No KL constraint (neither in reward nor as loss term)
#   - Token-level loss (token-mean aggregation)
#   - Dynamic Sampling: filter all-correct / all-incorrect groups
#   - Overlong Reward Shaping: linear penalty for overlong responses
#
# Tuned for 4B model on H100 80GB x8:
#   - gpu_memory_utilization=0.6  (vs 0.75 for 1.7B on H200)
#   - train_prompt_bsz=128, train_prompt_mini_bsz=16  (half of 1.7B to fit 4B weights+optimizer)
#
# Usage:
#   bash run_qwen3_4b_dapo.sh
...
project_name='verl'
exp_name="Qwen3-4B-DAPO-nothink"
NNODES=${NNODES:-1}
...
# DAPO Clip-Higher: asymmetric clip ratios (high > low encourages exploration)
clip_ratio_low=0.2
