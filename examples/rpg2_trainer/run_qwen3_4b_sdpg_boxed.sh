#!/bin/bash
# SDPG training script for Qwen3-4B on math reasoning tasks.
#
...
# Use a smaller ppo_micro_batch_size_per_gpu (e.g. 2) and lower
# gpu_memory_utilization (0.6) compared to plain GRPO.
#
# Usage:
#   BETA=0.001 ALPHA=0.001 KL_MODE=urkl bash run_qwen3_4b_sdpg_boxed.sh
#
...
exp_name="Qwen3-4B-SDPG-${KL_MODE}-BETA${BETA}-ALPHA${ALPHA}-cliphigh"
NNODES=${NNODES:-1}
...
rollout_engine=vllm
rollout_mode=async
# 0.75 provides sufficient headroom for (B, T, V) actor+teacher logits combined with
# ref model FSDP shards. If generate_sequences is slow, lower to 0.6.
