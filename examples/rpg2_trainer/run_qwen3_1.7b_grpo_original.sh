#!/bin/bash
# Original GRPO training script for Qwen3-1.7B on math reasoning tasks.
# Implements the original GRPO algorithm from DeepSeekMath (Shao et al., 2024):
#   - Standard PPO clip (no dual-clip, clip_ratio_c=1e9)
#   - KL penalty in reward (not as a loss term), kl_coef=1e-3
#   - seq-mean-token-mean loss aggregation (average over responses, then over tokens)
#   - No filter_groups (no dynamic sampling)
#   - No overlong buffer penalty
#
# Usage:
#   bash run_qwen3_1.7b_grpo_original.sh
...
project_name='verl'
exp_name="Qwen3-1.7B-GRPO-original"
NNODES=${NNODES:-1}
...
kl_coef=1e-3           # KL penalty coefficient
use_kl_loss=False
entropy_coeff=0
...
# Original GRPO: no overlong buffer
enable_overlong_buffer=False

# Original GRPO: seq-mean-token-mean (average over G responses, each averaged over tokens)
